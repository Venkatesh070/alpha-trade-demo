...
async function parseResponse<T>(res: Response): Promise<T & { error?: string; message?: string }> {
  try {
    ...
    if (res.status === 502) {
      throw new AuthApiError(
        502,
        "Server could not send the OTP email. Check Resend settings on Render (RESEND_API_KEY and RESEND_FROM_EMAIL).",
      );
    }
...
async function requestPublic<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    throw new AuthApiError(res.status, data.error ?? data.message ?? "Request failed.");
  }
