...
const STEPS = [
  { id: "identity", title: "Identity verification", desc: "Upload Aadhaar or Passport — front and back." },
  { id: "address", title: "Proof of address", desc: "Recent utility bill or bank statement (within 3 months)." },
...
function VerificationPage() {
  ...
  return (
    <PageShell
      ...
      width="md"
    >
      <DataPanel title="Verification progress" description={`${done.size} of ${STEPS.length} steps complete`}>
        <div className="mb-2 flex items-center justify-between text-xs">
          <span className="text-muted-foreground">Overall progress</span>
          <span className="font-mono font-semibold text-[color:var(--gold)]">{progress}%</span>
        </div>
        <div className="h-2 overflow-hidden rounded-full bg-surface">
          <div className="h-full rounded-full bg-gradient-to-r from-[color:var(--gold-deep)] to-[color:var(--gold)] transition-all" style={{ width: `${progress}%` }} />
        </div>
      </DataPanel>
          ...
          >
            <div className="flex flex-1 items-center gap-4">
              <div className={cn(
                ...
                done.has(s.id) ? "bg-[color:var(--success)] text-[color:var(--success-foreground)]" : "bg-[color:var(--gold)]/15 text-[color:var(--gold)]",
              )}>
            ...
            <Button
              ...
              className={cn("shrink-0 sm:w-auto", !done.has(s.id) && "gold-button hover:gold-button-hover")}
            >
      ...
      <div className="glossy-soft flex items-start gap-3 rounded-2xl p-4 text-sm text-muted-foreground">
        ...
        <p>Documents are encrypted at rest. Demo KYC is simulated — no real identity data is transmitted.</p>
      </div>
    </PageShell>
  );
}
