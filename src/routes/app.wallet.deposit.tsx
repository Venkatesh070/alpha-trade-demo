...
function DetailRow({ label, value, mono, onCopy }: { label: string; value: string; mono?: boolean; onCopy?: () => void }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-border/60 bg-surface/50 px-3 py-2.5">
      ...
      {onCopy && (
        <button type="button" onClick={onCopy} className="shrink-0 rounded-md border border-border p-1.5 text-muted-foreground hover:text-foreground" aria-label={`Copy ${label}`}>
...
function DepositPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!utr.trim()) { toast.error("Enter the UTR / transaction reference number"); return; }
    if (!screenshot) { toast.error("Upload a payment screenshot"); return; }
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="space-y-5">
            ...
            {[1000, MIN_TRADING_BALANCE, 10000, 25000, 50000].map((v) => (
              <button
                ...
                className={cn(
                  ...
                  amt === v ? "border-[color:var(--gold)] bg-[color:var(--gold)]/10 text-[color:var(--gold)]" : "border-border bg-surface hover:bg-accent",
                )}
              >
                ₹{v.toLocaleString()}
              </button>
        ...
        <DataPanel title="Step 2 · Pay using these details" padding={false}>
          ...
          {!hasPaymentConfig ? (
            <div className="rounded-xl border border-dashed border-border bg-surface/50 p-8 text-center text-sm text-muted-foreground">
              Payment QR and UPI details are not set up yet. Please try again later or contact support.
            </div>
          ) : (
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="flex flex-col items-center justify-center rounded-xl border border-border/60 bg-surface/30 p-5">
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Scan QR code</div>
                {settings.qrImage ? (
                  <img
                    src={settings.qrImage}
                    alt="UPI payment QR code"
                    className="mt-3 w-full max-w-[240px] rounded-xl border-2 border-[color:var(--gold)]/30 bg-white p-2 shadow-lg"
                  />
                ) : (
                ...
                <p className="mt-3 text-center text-xs text-muted-foreground">Open any UPI app and scan to pay</p>
              </div>
              ...
              <div className="space-y-3">
                <DetailRow
                  ...
                  onCopy={settings.accountName ? () => copy(settings.accountName, "Account name") : undefined}
                />
                <DetailRow
                  label="UPI ID"
                ...
                <p className="text-xs text-muted-foreground">
                  Pay exactly this amount. After payment, note the <strong className="text-foreground">UTR / reference number</strong> from your UPI app.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <Input
                ...
                required
              />
              <p className="mt-1.5 text-xs text-muted-foreground">12-digit UTR from GPay, PhonePe, Paytm, etc.</p>
            </div>

            <div>
              <Label>Payment screenshot</Label>
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => onScreenshot(e.target.files?.[0])} />
              {screenshot ? (
                <div className="mt-1.5 space-y-2">
                  <div className="overflow-hidden rounded-lg border border-border">
                    <img src={screenshot} alt="Payment screenshot" className="max-h-44 w-full object-contain bg-black/20" />
                  </div>
          ...
          <DataPanel title="Deposit summary">
            <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
              ...
              <div className="flex justify-between gap-2 sm:block">
                ...
                <dd className="flex items-center gap-1 truncate"><User className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />{settings.accountName || "—"}</dd>
              </div>
              <div className="flex justify-between gap-2 sm:block">
        ...
        <Button disabled={loading || !hasPaymentConfig} className="gold-button hover:gold-button-hover w-full py-6 text-base font-semibold">
