...
function LeaderboardsPage() {
  return (
    <PageShell
      eyebrow="Rankings"
      title="Leaderboards"
      description="Top 50 accounts by 30-day P&L. Rankings refresh every minute."
      width="lg"
    >
      <PriceLockBanner />
              ...
              <DataTableRow key={e.rank}>
                ...
                <Td
                  ...
                  className={
                    ...
                    (e.roi >= 0 ? "text-[color:var(--success)]" : "text-[color:var(--destructive)]")
                  }
                >
                  <GatedNumber
                    value={`${e.roi.toFixed(1)}%`}
                    className={
                      e.roi >= 0 ? "text-[color:var(--success)]" : "text-[color:var(--destructive)]"
                    }
                  />
                ...
                <Td mono className="text-right font-semibold">
                  <GatedNumber value={`₹${e.pnl.toLocaleString()}`} />
                </Td>
              </DataTableRow>
            ))}
          </tbody>
