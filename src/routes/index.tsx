...
import { ArrowRight, Zap, TrendingDown, Wallet, Users, GraduationCap, BarChart3, ShieldCheck, Globe2, Check, Star } from "lucide-react";
...
const ICON_MAP = { Zap, TrendingDown, Wallet, Users, GraduationCap, BarChart3, ShieldCheck, Globe2 } as const;

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Exness India — Trade Forex, Crypto, Gold & Stocks (Demo)" },
      { name: "description", content: "Premium demo trading platform. Razor-thin spreads from 0.0 pips, 2000:1 leverage, instant UPI deposits, and copy trading." },
      { property: "og:title", content: "Exness India — Premium Demo Trading Platform" },
      { property: "og:description", content: "200+ markets, sub-15ms execution, copy trading and a full trading academy. Open a free demo account." },
    ],
  }),
    ...
    <div className="relative min-h-screen">
          ...
          <div className="lg:col-span-7">
            <div>
              ...
              <h1 className="mt-6 font-display text-5xl font-extrabold leading-[1.05] tracking-tight sm:text-6xl lg:text-7xl">
                Trade <span className="gold-text">200+ markets</span><br />
                like an institution.
              </h1>
              <p className="mt-6 max-w-xl text-base text-muted-foreground sm:text-lg">
                Sub-15ms execution, spreads from 0.0 pips, instant UPI deposits and full copy trading — all in a polished terminal built for the Indian market. <strong className="text-foreground">100% simulated demo.</strong>
              </p>
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Button asChild size="lg" className="gold-button hover:gold-button-hover h-12 px-6 text-sm font-semibold">
                  <Link to="/register">Open Demo Account <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
                <Button asChild size="lg" variant="ghost" className="h-12 border border-border bg-surface/60 backdrop-blur hover:bg-accent">
          ...
          <BentoCard className="md:col-span-2 md:row-span-2">
            <div className="flex h-full flex-col justify-between gap-6">
              <div>
                ...
                <h3 className="mt-3 font-display text-2xl font-bold sm:text-3xl">A trading terminal that breathes.</h3>
                <p className="mt-3 max-w-md text-sm text-muted-foreground">Lightweight candlesticks, 30+ indicators, multi-chart layouts, hotkeys and one-click execution. Engineered for traders who measure latency in milliseconds.</p>
              </div>
              <FakeChartPreview />
            </div>
          </BentoCard>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Button asChild variant="ghost" className="text-[color:var(--gold)]">
            <Link to="/markets">All markets <ArrowRight className="ml-1 h-4 w-4" /></Link>
          </Button>
        </div>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((sym) => {
            ...
            return (
              <div key={sym} className="glossy group rounded-2xl p-5 transition-transform hover:-translate-y-0.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right font-mono">
                    <div className="text-base">{a.price.toLocaleString()}</div>
                    <div className={up ? "text-xs text-[color:var(--success)]" : "text-xs text-[color:var(--destructive)]"}>
                      {up ? "+" : ""}{a.changePct.toFixed(2)}%
                    </div>
                  </div>
                </div>
                <div className="mt-4"><Sparkline points={points} up={up} className="h-16 w-full" /></div>
                <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
                  <span>Spread {a.spread}p · 1:{a.leverage}</span>
                  <Link to="/trading" className="text-[color:var(--gold)] hover:underline">Trade →</Link>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {PRICING.map((p) => (
            <div key={p.name} className={"relative rounded-2xl p-6 " + (p.featured ? "glossy ring-gold" : "glossy-soft")}>
              {p.featured && (
                <span className="absolute -top-3 left-6 rounded-full bg-[color:var(--gold)] px-3 py-1 text-xs font-semibold text-[color:var(--primary-foreground)]">Most popular</span>
              )}
              <div className="font-display text-xl font-bold">{p.name}</div>
              <div className="text-sm text-muted-foreground">{p.tagline}</div>
              <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                <div><div className="text-muted-foreground text-xs">Spread</div><div className="font-mono">{p.spread}</div></div>
                <div><div className="text-muted-foreground text-xs">Commission</div><div className="font-mono">{p.commission}</div></div>
                <div><div className="text-muted-foreground text-xs">Leverage</div><div className="font-mono">{p.leverage}</div></div>
                <div><div className="text-muted-foreground text-xs">Min deposit</div><div className="font-mono">{p.minDeposit}</div></div>
              </div>
              <ul className="mt-6 space-y-2 text-sm">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><Check className="mt-0.5 h-4 w-4 text-[color:var(--gold)]" /><span>{f}</span></li>
                ))}
              </ul>
          ...
          {TESTIMONIALS.map((t) => (
            <div key={t.id} className="glossy-soft rounded-2xl p-5">
              ...
              <div className="mt-4 flex items-center gap-3">
                <div className="grid h-8 w-8 place-items-center rounded-full bg-[color:var(--gold)] text-xs font-bold text-[color:var(--primary-foreground)]">{t.name[0]}</div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-6 text-center">
          <Button asChild variant="ghost"><Link to="/faq" className="text-[color:var(--gold)]">View all FAQs →</Link></Button>
        </div>
      </section>
      ...
      <section className="mx-auto my-10 max-w-7xl px-4 sm:px-6">
        <div className="glossy relative overflow-hidden rounded-3xl p-10 text-center sm:p-16">
          <h2 className="font-display text-3xl font-extrabold sm:text-4xl">Ready to <span className="gold-text">trade smarter?</span></h2>
          <p className="mx-auto mt-3 max-w-xl text-muted-foreground">Open a demo account in 30 seconds. ₹10,00,000 virtual balance, instant access to every market.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Button asChild size="lg" className="gold-button hover:gold-button-hover h-12 px-6 text-sm font-semibold">
              <Link to="/register">Open Demo Account</Link>
            </Button>
            <Button asChild size="lg" variant="ghost" className="h-12 border border-border bg-surface/60">
    ...
    <div className="glossy relative rounded-3xl p-4">
      <div className="flex items-center justify-between p-2">
        <div>
          ...
          <div className="font-display text-2xl font-bold">$68,420.<span className="text-muted-foreground">50</span></div>
        </div>
        <div className="text-right text-xs">
          <div className="rounded-md bg-[color:var(--success)]/20 px-2 py-1 font-mono text-[color:var(--success)]">+2.41%</div>
        </div>
      </div>
      <div className="relative h-64 w-full rounded-xl bg-[color:var(--surface-2)]/60 p-2">
        ...
        <div className="pointer-events-none absolute inset-0 rounded-xl" style={{
          background: "radial-gradient(60% 40% at 80% 20%, rgba(250,204,21,0.18), transparent 60%)"
        }} />
      </div>
      <div className="mt-3 grid grid-cols-3 gap-2 px-2 text-[11px] text-muted-foreground">
        <div>Open<div className="font-mono text-foreground">67,210</div></div>
        <div>High<div className="font-mono text-foreground">68,612</div></div>
        <div>Low<div className="font-mono text-foreground">66,940</div></div>
      </div>
    </div>
  );
}
