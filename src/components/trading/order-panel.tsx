...
import { GatedPriceText } from "@/components/pricing/price-gate";
...
export function OrderPanel({
  ...
  const submit = (side: "buy" | "sell") => {
    ...
    toast.success(`${side.toUpperCase()} ${q} ${symbol} @ ${formatPrice(asset, px)}`, {
      description: `${type.toUpperCase()} • leverage 1:${leverage}`,
    });
  };

  return (
    <div
      className={cn("flex h-full flex-col gap-3 p-3", disabled && "pointer-events-none opacity-60")}
    >
      <div>
        ...
        <div className="mt-1 flex items-baseline justify-between">
          ...
          <GatedPriceText asset={asset} price={price} className="text-sm" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 rounded-md border border-border bg-surface p-1 text-xs">
        {(["market", "limit", "stop"] as OrderType[]).map((t) => (
          <button
            ...
            onClick={() => setType(t)}
            className={cn(
              "rounded px-2 py-1.5 font-medium capitalize transition-colors",
              type === t
      ...
      {type !== "market" && (
        <div>
          ...
          <Input
            ...
            placeholder={disabled ? "—" : String(formatPrice(asset, price))}
            className="font-mono"
          />
        </div>
      )}
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 pt-2">
        <Button
          onClick={() => submit("sell")}
          className="bg-[color:var(--destructive)] text-[color:var(--destructive-foreground)] hover:brightness-110"
        >
          Sell
        </Button>
        <Button
      ...
      <div className="grid grid-cols-2 gap-3 rounded-md border border-border bg-surface px-3 py-2 text-[11px] text-muted-foreground">
        <div>
          Spread<div className="font-mono text-foreground">{asset.spread} pips</div>
        </div>
        <div>
          Margin
          <div className="font-mono text-foreground">
            {disabled ? (
              ...
              <>≈ {((Number(qty || 0) * price) / leverage).toFixed(2)}</>
            )}
          </div>
        </div>
      </div>
