...
export function AppTopbar({ onMenu }: { onMenu: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border/60 bg-background/70 px-4 backdrop-blur-xl">
      <button onClick={onMenu} className="grid h-9 w-9 place-items-center rounded-md border border-border/60 lg:hidden" aria-label="Menu">
      ...
      <div className="ml-auto flex items-center gap-2">
        <button onClick={toggle} aria-label="Theme" className="grid h-9 w-9 place-items-center rounded-md border border-border/60">
          {theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
        </button>
        <Link to="/app/notifications" className="relative grid h-9 w-9 place-items-center rounded-md border border-border/60">
          <Bell className="h-4 w-4" />
          <span className="absolute -right-0.5 -top-0.5 grid h-4 w-4 place-items-center rounded-full bg-[color:var(--gold)] text-[10px] font-bold text-[color:var(--primary-foreground)]">2</span>
        </Link>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => nav({ to: "/app/profile" })}><User2 className="mr-2 h-4 w-4" /> Profile</DropdownMenuItem>
